...
<h3> {{__('Edit')}} {{__('Headers')}}</h3>
@endsection

@section('breadcrumb-items')
<li class="breadcrumb-item"> {{__('Headers')}}</li>
<li class="breadcrumb-item active"> {{__('Edit')}} {{__('Headers')}}</li>
@endsection
            ...
            <div class="col-xl-12">
                <form method="POST" action="{{ route('header.update', $header->id) }}" enctype="multipart/form-data"
                    ...
                    <div class="card-header">
                        {{-- <h4 class="card-title mb-0">{{ __('Edit Header') }}</h4> --}}
                                ...
                                <div class="col-md-12">
                                    ...
                                    <select required name="country_id[]" id="country_id" class="form-control select2"
                                        ...
                                        <option value="{{ $country->id }}" {{ in_array($country->id,
                                            $header->country_id) ? 'selected' : '' }}>{{ $country->name }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="checkbox" id="checkbox2" >{{ __('All') }}
                                    ...
                                    <select required name="city_id[]" id="city_id" class="form-control select2"
                                        ...
                                        <option value="{{ $city->id }}" {{ in_array($city->id, $header->city_id) ?
                                    ...
                                    <div id="idiom-{{$array_data[$i][0]->idiom_id}}">
                                        ...
                                        <div class="row">
                                            <div class="col-md-6 mb-3">
                                                ...
                                                <div class="col-md-12">
                                                    <input id="subtitle_{{$array_data[$i][0]->idiom_id}}" type="text"
                                                        ...
                                                        name="subtitle_{{$array_data[$i][0]->idiom_id}}"
                                                ...
                                                <br /> 
                                                <label for="link_{{$array_data[$i][0]->idiom_id}}"
                                                    class="form-label">{{ __('Link') }}</label>

                                                @if(isset($array_data[$i][0]->link))
                                                    <div class="col-md-12">
                                                        <input id="link_{{$array_data[$i][0]->idiom_id}}" type="text"
                                                            class="form-control @error('link_es') is-invalid @enderror"
                                                            name="link_{{$array_data[$i][0]->idiom_id}}"
                                                            value="{{$array_data[$i][0]->link}}">
                                            ...
                                            <div class="col-md-6 mb-3">
                                                ...
                                                <div class="col-md-12">
                                                    @if($array_data[$i][0]->image)
                                                    <img id="image-preview{{$array_data[$i][0]->idiom_id}}" src="{{$array_data[$i][0]->image }}"
...
function updateFields() {
    ...
    $('#idiom-fields > div').each(function() {
        ...
        if (selectedIdioms.indexOf(id) === -1) {
            ...
            let subtitle = document.getElementById("subtitle_" + id).value = null;
